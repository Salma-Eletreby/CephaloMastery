...
/**
 * Question type class for diagnosis is defined here.
 *
...
class qtype_diagnosis extends question_type {
    public function get_question_options($question) {
        global $DB, $OUTPUT;

        $question->options = $DB->get_record('qtype_diagnosis_options', ['questionid' => $question->id]);
    ...
    protected function create_default_options($question) {
        ...
        $config = get_config('qtype_diagnosis');
    ...
    public function save_question_options($question) {
        ...
        if ($answercount < 2) { // Check there are at lest 2 answers for multiple choice.
            $result->error = get_string('notenoughanswers', 'qtype_diagnosis', '2');
            return $result;
        }
        ...
        $options = $DB->get_record('qtype_diagnosis_options', array('questionid' => $question->id));
        if (!$options) {
            $options = new stdClass();
            $options->questionid = $question->id;
            ...
            $options->showstandardinstruction = 0;
            $options-> questiontype= $question->questionType;
            $options->id = $DB->insert_record('qtype_diagnosis_options', $options);
        }
        ...
        $options-> questiontype= $question->questionType;
        $DB->update_record('qtype_diagnosis_options', $options);
        ...
        if ($options->single) {
            if ($maxfraction != 1) {
                $result->noticeyesno = get_string('fractionsnomax', 'qtype_diagnosis',
                        $maxfraction * 100);
                return $result;
            }
        } else {
            ...
            if ($totalfraction != 1) {
                $result->noticeyesno = get_string('fractionsaddwrong', 'qtype_diagnosis',
                        $totalfraction * 100);
                return $result;
            }
        }
    ...
    protected function make_question_instance($questiondata) {
        ...
        if ($questiondata->options->single) {
            $class = 'qtype_diagnosis_single_question';
        } else {
            $class = 'qtype_diagnosis_multi_question';
        }
        return new $class();
    }
    ...
    protected function initialise_question_instance(question_definition $question, $questiondata) {
        ...
        if (!empty($questiondata->options->layout)) {
            ...
            $question->layout = qtype_diagnosis_single_question::LAYOUT_VERTICAL;
        }
    ...
    public function delete_question($questionid, $contextid) {
        global $DB;
        $DB->delete_records('qtype_diagnosis_options', array('questionid' => $questionid));
     ...
     *      should be a lang string answernumberingxxx in teh qtype_diagnosis
    ...
    public static function get_numbering_styles() {
        ...
        foreach (array('abc', 'ABCD', '123', 'iii', 'IIII', 'none') as $numberingoption) {
            $styles[$numberingoption] =
                    get_string('answernumbering' . $numberingoption, 'qtype_diagnosis');
        }
        return $styles;
    }

    //Added
    public static function get_questionMode() {
        $styles = array();
        foreach (array('None','EOFS','EOFAS','EOFLVH','EOFLC','EOFSSL','EOFSSA','EOFSDM','EOFSDMSD','EOFSDMSDD','EOFSOC',
        'PD','PNA','PMPA','PLFH') as $qmode) {
            $styles[$qmode] =
                    get_string('questionType' . $qmode, 'qtype_diagnosis');
        }
        return $styles;
    }
