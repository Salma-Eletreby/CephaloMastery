 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_landmarks_renderer extends qtype_ddtoimage_renderer_base
{
    public function formulation_and_controls(
        ...
        $output .= html_writer::img(
            self::get_url_for_image($qa, 'bgimage'),
            get_string('dropbackground', 'qtype_landmarks'),
        ...
        if ($question->showmisplaced && $qa->get_state()->is_finished()) {
            ...
            if (count($wrongparts) !== 0) {
                ...
                $output .= html_writer::span(
                    get_string('followingarewrongandhighlighted', 'qtype_landmarks', $wrongpartsstring),
                    'wrongparts'
                );
            }
        }
    ...
    protected function hint(question_attempt $qa, question_hint $hint)
    {
        ...
        if ($hint->statewhichincorrect) {
            ...
            foreach ($wrongdrags as $wrongdrag) {
                $wrongparts[] = html_writer::nonempty_tag(
                    'span',
                    $wrongdrag,
                    array('class' => 'wrongpart')
                );
            }
            $output .= html_writer::nonempty_tag(
                'div',
                get_string('followingarewrong', 'qtype_landmarks', join(', ', $wrongparts)),
