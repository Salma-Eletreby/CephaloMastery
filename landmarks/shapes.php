 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class qtype_landmarks_shape
{
    /** @var bool Indicates if there is an error */
    protected $error = false;

    /** @var string The shape class prefix */
    protected static $classnameprefix = 'qtype_landmarks_shape_';
    ...
    public function inside_width_height($widthheight)
    {
        foreach ($this->outlying_coords_to_test() as $coordsxy) {
            if (
                $coordsxy[0] < 0 || $coordsxy[0] > $widthheight[0] ||
    ...
    public function get_coords_interpreter_error()
    {
        if ($this->error) {
            ...
            return get_string('formerror_' . $this->error, 'qtype_landmarks', $a);
        } else {
            return false;
        }
    }
    ...
    public static function human_readable_name($lowercase = false)
    {
        ...
        return get_string($stringid, 'qtype_landmarks');
    }

    public static function human_readable_coords_format()
    {
        return get_string('shape_' . self::name() . '_coords', 'qtype_landmarks');
    }
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_landmarks_shape_rectangle extends qtype_landmarks_shape
{
    ...
    {
        return $this->is_point_in_bounding_box(
            $xy,
            array($this->xleft, $this->ytop),
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_landmarks_shape_circle extends qtype_landmarks_shape
{
    /** @var int X center */
    protected $xcentre;
    ...
    /** @var double penalty */
    public static $penalty = 0.0;

    //added - Salma Eletreby
    /** @var array array with points that are penalty */
    public static $pnArray = array();

    public function __construct($coordsstring = null)
    {
...
{        $coordstring = preg_replace('!\s!', '', $coordsstring);
    ...
    //Added - Salma Eletreby
    public static function incrementPenalty($amount)
    {
        self::$penalty += $amount;
    }

    //Added - Salma Eletreby
    public static function addToPenalty($x)
    {
        self::$pnArray[] = $x;
    }

    public static function clearPenalty()
    {
        self::$penalty =0;
        self::$pnArray = array();
    }
    ...
    public function is_point_in_shape($xy)
    {
        ...
        $distanceInpx = floatval($distancefromcentre) * 0.0264583333;

        //Added - Salma Eletreby
        if ($distancefromcentre <= $this->radius && $distanceInpx > 0.2) {
            if (in_array($this->xcentre, self::$pnArray) == false) {
                self::incrementPenalty(round($distanceInpx, 1));
                self::addToPenalty($this->xcentre);
            }
        }

        if(in_array($this->xcentre, self::$pnArray) == true){
            self::clearPenalty();
        }
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_landmarks_shape_polygon extends qtype_landmarks_shape
{
    /**
    ...
    public function __construct($coordsstring)
    {
        ...
        if (count($coordsstringparts) < 3) {
            ...
            if (
                $this->coords[count($this->coords) - 1][0] == $this->coords[0][0] &&
    ...
    public function is_point_in_shape($xy)
    {
        ...
        $point = new qtype_landmarks_point($xy[0], $xy[1]);
        $windingnumber = 0;
        foreach ($this->coords as $index => $coord) {
            $start = new qtype_landmarks_point($this->coords[$index][0], $this->coords[$index][1]);
            if ($index < count($this->coords) - 1) {
                $endindex = $index + 1;
            } else {
                $endindex = 0;
            }
            $end = new qtype_landmarks_point($this->coords[$endindex][0], $this->coords[$endindex][1]);
     ...
     *
     * @param qtype_landmarks_point $start first of two points on the infinite line.
     * @param qtype_landmarks_point $end second of two points on the infinite line.
     * @param qtype_landmarks_point $point the oint to test.
     * @return number > 0 if the point is left of the line.
     *                 = 0 if the point is on the line.
     *                 < 0 if the point is right of the line.
     */
    protected function is_left(
        qtype_landmarks_point $start,
        qtype_landmarks_point $end,
        qtype_landmarks_point $point
    ) {
    ...
    {
        $center = array(
            round(($this->minxy[0] + $this->maxxy[0]) / 2),
