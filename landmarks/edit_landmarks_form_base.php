 ...
 */
abstract class qtype_landmarks_edit_form_base extends question_edit_form
{
    /**
     * (MODIFIED) Maximum number of different groups of drag items there can be in a question.
     */
    const MAX_GROUPS = 25;
    ...
    /**
     * (Added) Settings for the JSON file picker 
     *
     * @return array Array of filepicker options for the JSON file.
     */
    public static function JSON_file_picker_options()
    {
        $filepickeroptions = array();
        $filepickeroptions['accepted_types'] = array('application/json');
        $filepickeroptions['maxbytes'] = 0;
        $filepickeroptions['maxfiles'] = 1;
        $filepickeroptions['subdirs'] = 0;
        return $filepickeroptions;
    ...
    protected function definition_inner($mform)
    {
        ...
        $mform->addElement(
            'filepicker',
            'bgimage',
            get_string('bgimage', 'qtype_' . $this->qtype()),
            null,
            self::file_picker_options()
        );

        $mform ->addElement('text','userToken','Token');
        $mform->addHelpButton('userToken', 'userToken', 'qtype_landmarks');
        $mform->addElement('button', 'generateML', get_string('generate', 'qtype_'.$this->qtype()));
        
        $attributes = array(
            'data-skip-validation' => '1',
            'data-no-submit' => '1',
        );
        $mform->getElement('generateML')->updateAttributes($attributes);
        //Added
        // $mform->registerNoSubmitButton('JSON');
        // $mform->addElement('submit', 'JSON', get_string('JSON', 'qtype_'.$this->qtype()));
        $mform->addElement('button', 'JSON', get_string('JSON', 'qtype_' . $this->qtype()));
        $mform->addElement(
            'filepicker',
            'jsonFile',
            get_string('JSONFile', 'qtype_' . $this->qtype()),
            null,
            self::JSON_file_picker_options()
        );
        $mform->addElement('static','JSONLoading',"");
        $mform->getElement('JSON')->updateAttributes(array('onclick' => '
        var filePicker = document.getElementById(\'fitem_id_jsonFile\');
            if (window.getComputedStyle(filePicker).display == \'none\') {
                filePicker.style.display = \'block\'; // Show the file picker
                document.querySelectorAll(\'.felement[data-fieldtype="filepicker"]\')[1].style.marginLeft = \'18rem\';
                document.querySelectorAll(\'.felement[data-fieldtype="filepicker"]\')[1].style.marginTop = \'-2rem\';
            } else {
        ...
        //Added
        $this->definition_distance_items($mform, $itemrepeatsatstart, $imagerepeats);
        //Aya
        $this->definition_angle_items($mform, $itemrepeatsatstart, $imagerepeats);
    ...
    protected function definition_drop_zones($mform, $imagerepeats)
    {
        ...
        $this->repeat_elements(
            $this->drop_zone($mform, $imagerepeats),
